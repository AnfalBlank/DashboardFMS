...
import { Bell, Search, X, ChevronDown, User, Settings, LogOut, Check } from 'lucide-react';
...
export function Header() {
  ...
  const critical = alerts.filter(a => a.type === 'CRITICAL' || a.severity === 'CRITICAL').length;
  const warning = alerts.filter(a => a.type === 'WARNING' || a.severity === 'WARNING').length;
    ...
    <>
      <header className="sticky top-0 z-40 h-[60px] bg-white border-b border-slate-200 flex items-center px-6 gap-4 shadow-sm">
        ...
        <div className="flex items-center gap-2.5 ml-auto">
          ...
          {/* Date */}
          <div className="text-[12px] text-slate-500 font-medium hidden lg:block">
              ...
              <div className="absolute right-0 top-11 w-[360px] bg-white border border-slate-200 rounded-2xl shadow-[0_20px_60px_-10px_rgba(0,0,0,.18)] z-50 animate-fade-in overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
                  <div className="flex items-center gap-2">
                    ...
                    {critical > 0 && <Badge variant="critical">{critical} kritis</Badge>}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-slate-600 w-6 h-6 flex items-center justify-center rounded-full hover:bg-slate-200 transition"
                    >
                      <X size={13} />
                    </button>
                  </div>
                ...
                <div className="max-h-[360px] overflow-y-auto">
                  {alerts.length === 0 ? (
                    <div className="px-4 py-8 text-center text-[13px] text-slate-400">Tidak ada notifikasi aktif</div>
                  ) : (
                    alerts.map(a => (
                      <div
                        key={a.id}
                        className="flex gap-3 px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition last:border-0 group cursor-pointer"
                      >
                        <div className="flex-shrink-0 pt-1.5">
                          <span
                            className={`inline-block w-2 h-2 rounded-full ${
                              a.type === 'CRITICAL' || a.severity === 'CRITICAL' ? 'bg-red-500' :
                              a.type === 'WARNING' || a.severity === 'WARNING' ? 'bg-amber-500' : 'bg-blue-500'
                            }`}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          {(a.message || a.desc) && (
                            <p className="text-[12px] text-slate-500 mt-0.5 leading-snug">{a.message || a.desc}</p>
                          )}
                        </div>
                        <button
                          onClick={(e) => { e.stopPropagation(); handleMarkRead(a.id); }}
                          title="Tandai dibaca"
                          className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-slate-700 p-1"
                        >
                          <Check size={12} />
                        </button>
                      </div>
                ...
                <Link href="/system/audit" onClick={() => setNotifOpen(false)}>
                  <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50 hover:bg-slate-100 transition cursor-pointer">
                    <p className="text-[12.5px] text-slate-500 font-medium text-center">Lihat semua audit trail →</p>
                  </div>
                </Link>
              </div>
            )}
          ...
          <div className="relative" ref={userRef}>
            ...
            >
              ...
              <span className="text-[13px] font-medium text-slate-700">{user?.username ?? 'ADMIN01'}</span>
            ...
            {userOpen && (
              <div className="absolute right-0 top-11 w-[220px] bg-white border border-slate-200 rounded-xl shadow-[0_10px_40px_-8px_rgba(0,0,0,.18)] z-50 animate-fade-in overflow-hidden">
                <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
                  <p className="text-[13px] font-semibold text-slate-800">{user?.name ?? 'Administrator'}</p>
                  <p className="text-[12px] text-slate-400">{user?.role ?? 'Super Admin'}</p>
                </div>
                <div className="py-1">
                  {[
